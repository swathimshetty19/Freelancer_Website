...
def post_project(request,pk):
    if request.method == "POST": 
            ...
            form=Projects(project_id=project_id,name=name,desc=desc,status=status,skills=skills,mem_needed=mem_needed,min_payment=min_payment,max_payment=max_payment,days=days,leader=pk)
            form.save()
...
def myproj(request,pk,proj):
    ...
    return render(request, 'myproj.html', {'pk':pk,'proj':pr,'applied':applied,"participants":members})
